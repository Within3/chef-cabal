...
def install_by_name
  ...
  execute "su - #{new_resource.user} -c '#{cabal_command} #{new_resource.package_name}'" do
...
def install_by_path(path)
  run_cabal_update if new_resource.cabal_update
  execute "su - #{new_resource.user} -c 'cd #{path} && #{cabal_command(new_resource.cabal_dev)}'" do
    returns new_resource.return_codes
  end
